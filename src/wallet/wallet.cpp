    ...
    for (auto pcoin : GetSpendableTXs()) {
        ...
        for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
            ...
            if (nCoinType == CoinType::ONLY_FULLY_MIXED) {
                ...
                found = pcoin->tx->vout[i].nValue == MN_COLLATERAL;
              if (chainActive.Height() >=  Params().GetConsensus().nPOWR && found) {
                found = pcoin->tx->vout[i].nValue == MN_COLLATERAL_2;
              }
    ...
    for (const auto& outpoint : setWalletUTXO) {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            if(fAnonymizable) {
                ...
                if (fMasternodeMode && wtx.tx->vout[i].nValue == MN_COLLATERAL_2 && chainActive.Height() >= Params().GetConsensus().nPOWR) continue;
