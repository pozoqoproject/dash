...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight >= nProofOfWorkReward) {
        ...
        if (dFactor < 1.0) {
            nSubsidy *= 1.0; // Set reward to 1000 PZQ for low difficulty
        } else if (dFactor > 8.0) {
            nSubsidy *= 4.0; // Set reward to 4000 PZQ for high difficulty
        } else {
            nSubsidy *= (dFactor / 2.0); // Linearly adjust reward between 1000 and 4000 PZQ
        }
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, *m_quorum_block_processor, state, view, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(POZQOQ): ProcessSpecialTxsInBlock for block %s failed with %s",
        ...
        for (const auto& tx : block.vtx) {
            ...
            while (llmq::CInstantSendLockPtr conflictLock = m_isman->GetConflictingLock(*tx)) {
                if (m_clhandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                    LogPrint(BCLog::ALL, "ConnectBlock(POZOQO): chain-locked transaction %s overrides islock %s\n",
                    ...
                    return state.Invalid(ValidationInvalidReason::TX_CONFLICT_LOCK, error("ConnectBlock(POZOQO): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()), REJECT_INVALID, "conflict-tx-lock");
                }
            }
        }
    }
    ...
    if (!IsBlockValueValid(*sporkManager, *governance, *::masternodeSync, block, pindex->nHeight, blockReward, strError)) {
        ...
        return state.Invalid(ValidationInvalidReason::NONE, error("ConnectBlock(POZOQO): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*sporkManager, *governance, *block.vtx[0], pindex->nHeight, blockReward)) {
        // NOTE: Do not punish, the node might be missing governance data
        return state.Invalid(ValidationInvalidReason::NONE, error("ConnectBlock(POZOQO): couldn't find masternode or superblock payments"), REJECT_INVALID, "bad-cb-payee");
    }
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs, const CChainParams& params)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, *m_quorum_block_processor, state, inputs, false /*fJustCheck*/, false /*fScriptChecks*/)) {
        return error("RollforwardBlock(POZOQO): ProcessSpecialTxsInBlock for block %s failed with %s",
            pindex->GetBlockHash().ToString(), FormatStateMessage(state));
    }
