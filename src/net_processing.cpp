 ...
 *  We have 4 times smaller block times in Pozoqo, so we need to push 4 times more invs per 1MB. */
...
bool static AlreadyHave(const CInv& inv, const CTxMemPool& mempool, const LLMQContext& llmq_ctx) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /*
        Pozoqo Related Inventory Messages
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    ...
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        // PZQ this code also handles non-TXs (Pozoqo specific messages)
